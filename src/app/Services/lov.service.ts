...
export class LovService {
  ...
  // These caches reduce redundant API calls for static LOV data.
  private categoriesCache$?: Observable<any>;
  private lovCache = new Map<number, Observable<any>>();

  /**
   * Clears all cached API responses to ensure fresh data.
   * Should be called when data is modified or user sessions change.
   */
  clearCache() {
  ...
  addlov(formattedData: any) {
    ...
    return this.http.post<any>(`${this.baseApiUrl}/lov/addlov`, body).pipe(
      tap(() => this.clearCache()) // Invalidate cache on modification
    );
  }
  ...
  logout() {
    ...
    this.clearCache(); // Security & Accuracy: Clear cache on logout
  }
}
