...
export class LovService {
  ...
  // Cache for categories and LOVs to optimize performance by reducing redundant API calls.
  ...
  getCategories() {
    ...
    this.categoriesCache$ = this.http.post<any>(this.baseApiUrl + '/LovType/findAllLovType', body).pipe(
      shareReplay(1),
      catchError((error) => {
        this.categoriesCache$ = null; // Clear cache on error so next attempt tries again
  ...
  getListOfValues(lovTypeId: number) {
    ...
    const request$ = this.http.post<any>(`${this.baseApiUrl}/lov/findAlllov`, body).pipe(
      shareReplay(1),
      catchError((error) => {
        this.lovCache.delete(lovTypeId); // Clear cache on error
        return throwError(() => error);
      }),
    );
  ...
  // Resets the cache to ensure fresh data is fetched after updates or logout.
  clearCache() {
